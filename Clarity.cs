...
public class Clarity
{
    ...
    // Declare the monitor as nullable.
    ...
    /// Skips the canvas named "Canvas" that is a direct child of "PlayerInteractionHelpUI".
    /// </summary>
    public static void ApplyFixToCanvas(Canvas canvas)
    {
        ...
        // Skip the canvas if it is named "Canvas"
        // and its immediate parent is "PlayerInteractionHelpUI".
        if (
            canvas.name == "Canvas"
            && canvas.transform.parent != null
            && canvas.transform.parent.name == "PlayerInteractionHelpUI"
        )
        {
            Debug.Log(
                $"[Clarity] Skipping canvas '{canvas.name}' under 'PlayerInteractionHelpUI'."
            );
            return;
        }
...
public class EnableUltrawideFixSetting : BoolSetting, IExposedSetting
{
    ...
    public LocalizedString GetDisplayName() => new UnlocalizedString("Enable Ultrawide Fix");
    ...
    public override void ApplyValue()
    {
        ...
        )
        {
            if (width > 0 && height > 0)
            {
                ...
                Debug.Log(
                    $"[Clarity] Screen resolution set to {width}x{height}, "
            ...
            {
                Debug.LogWarning(
                    $"[Clarity] Invalid resolution dimensions: "
        ...
        {
            Debug.LogWarning(
                $"[Clarity] Invalid resolution format: '{Value}'. " + "Expected 'WIDTHxHEIGHT'."
            );
        }
    }
